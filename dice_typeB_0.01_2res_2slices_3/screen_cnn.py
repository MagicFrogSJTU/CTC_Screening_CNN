...
def inference(volumes, iftrain):
    ...
    with tf.name_scope('down1'):
        conv1 = layer(conv, 'conv1.0', 32, [5,5,5], ifRelu=True)
        conv1 = layer(conv1, 'conv1.1', 32, [5,5,5], ifRelu=False)
        conv = conv1[:,:,:,:,0:1] + conv
        conv = tf.concat([conv, conv1[:,:,:,:,1:32]], 4)
        conv = tf.nn.leaky_relu(conv)

        conv1 = layer(conv, 'conv1.2', 32, [5, 5, 5], ifRelu=True)
        conv1 = layer(conv1, 'conv1.3', 32, [5, 5, 5], ifRelu=False)
        conv = conv1 + conv
        conv_left1 = tf.nn.leaky_relu(conv)

        conv = layer(conv_left1, 'down1', 32, [2,2,2], [2,2,2], ifRelu=True)

    # 2
    with tf.name_scope('down2'):
        conv1 = layer(conv, 'conv2.0', 32, [5,5,5], ifRelu=True)
        conv1 = layer(conv1, 'conv2.1', 32, [5,5,5], ifRelu=False)
        conv = conv1 + conv
        conv = tf.nn.leaky_relu(conv)

        conv1 = layer(conv, 'conv2.2', 32, [5, 5, 5], ifRelu=True)
        conv1 = layer(conv1, 'conv2.3', 32, [5, 5, 5], ifRelu=False)
        conv = conv1 + conv
    ...
    with tf.name_scope('up2'):
        ...
        conv1 = layer(conv1, 'conv2.0_', 32, [5,5,5], ifRelu=True)
        conv1 = layer(conv1, 'conv2.1_', 32, [5,5,5], ifRelu=False)
        conv = conv1 + conv
        conv = tf.nn.leaky_relu(conv)

        conv1 = layer(conv, 'conv2.2_', 32, [5, 5, 5], ifRelu=True)
        conv1 = layer(conv1, 'conv2.3_', 32, [5, 5, 5], ifRelu=False)
        conv = conv1 + conv
        conv = tf.nn.leaky_relu(conv)
